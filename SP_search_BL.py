...
def heimdall_run(fil_file,dmlo,dmhi,base_name,snr_cut,dorfi,kill_chan_range,heimdall):
	...
	if dorfi is True or manualzap:
		...
		cmd = "heimdall -f %s -rfi_tol 100 -dm_tol 1.001 -no_scrunching -dm_pulse_width 100 -rfi_no_narrow -rfi_no_broad -dm_nbits 32 -dm %f %f -boxcar_max %f -output_dir %s -v %s %s" % (fil_file,dmlo,dmhi,boxcar_max,outdir,zapchan,heimdall)		
		print cmd
		#os.system(cmd)
		...
		cmd = "heimdall -f %s -dm_tol 1.001 -no_scrunching -rfi_tol 100 -dm_pulse_width 100   -rfi_no_narrow -rfi_no_broad -dm_nbits 32 -dm %f %f -boxcar_max %f -output_dir %s -v %s" % (fil_file,dmlo,dmhi,boxcar_max,outdir,heimdall)	
		print cmd
		#os.system(cmd);
		p=sb.Popen(cmd,stdout=sb.PIPE, shell=True)
                out,err=p.communicate()
