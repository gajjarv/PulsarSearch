...
# Documentation can be found in Pulsar_seach_LOFATR.txt file
#
...
# 	Inpath and outpath can have / at the end or it can be without it. In both the cases it will process the script
	...
	# The second last element of indir is RSP 
	rsp = indir.split('/')[-2] 
        
	outfile = outdir + "Rfifind_" + rsp  
	...
	fp = open(inffile[0],"r")
  	data = fp.read().split("\n")
	fp.close()
	lofq = float(data[16].split("=")[1])
	bandwidth  = float(data[17].split("=")[1])
	chan = int(data[18].split("=")[1])
	samptime = float(data[11].split("=")[1])
	RA = (data[5].split("=")[1])
	DEC = (data[6].split("=")[1])
	PSR = (data[4].split("=")[1])
...
#	print inffile[0],infile,outfile
	...
	if(ddplanflag == 0):
		...
		cmd = "DDplan.py -f %s -b %s -n %s -t %f -r 0.2 -o %s > %s" % \
...
#        This is for hard coded DDplan for RSPA with HBA. Cant be used with mpiprepsubband  
	...
	print "\nLODM DMSTEP DMPERPASS DOWNSAMP NUMPASS"
        ...
        # rfifind (block ~2.6s)
        # does not work now because rfifind can't take more than 1024 files
...
#       Prepsubband outfile name
        ...
        prepsubbandout = outdir + "prepsubband_out_" + rsp
...
#	  Here for given number of DMs, it will round off to the nearest value where it matches with the number divisible
	...
	# Single pulse search
	cmd="single_pulse_search.py %s > %s" % (base + "_DM*.dat", outdir + "singlepulse.log")
